        ...
        public static string PinyinFirst(this string str)
        {
          ...
          "羧羯羰羲籼敉粑粝粜粞粢粲粼粽糁糇糌糍糈糅糗糨艮暨羿翎翕翥翡翦翩翮翳糸絷綦綮繇纛麸麴赳趄趔趑趱赧赭豇豉酊酐酎酏酤酢酡酰酩酯酽酾酲酴酹醌醅醐醍醑醢醣醪醭醮醯醵醴醺豕鹾趸跫踅蹙蹩趵趿趼趺跄跖跗跚跞跎跏跛跆跬跷跸跣跹跻跤踉跽踔踝踟踬踮踣踯踺蹀踹踵踽踱蹉蹁蹂蹑蹒蹊蹰蹶蹼蹯蹴躅躏躔躐躜躞豸貂貊貅貘貔斛觖觞觚觜觥觫觯訾謦靓雩雳雯霆霁霈霏霎霪霭霰霾龀龃龅龆龇龈龉龊龌黾鼋鼍隹隼隽雎雒瞿雠銎銮鋈錾鍪鏊鎏鐾鑫鱿鲂鲅鲆鲇鲈稣鲋鲎鲐鲑鲒鲔鲕鲚鲛鲞鲟鲠鲡鲢鲣鲥鲦鲧鲨鲩鲫鲭鲮鲰鲱鲲鲳鲴鲵鲶鲷鲺鲻鲼鲽鳄鳅鳆鳇鳊鳋鳌鳍鳎鳏鳐鳓鳔" + "鳕鳗鳘鳙鳜鳝鳟鳢靼鞅鞑鞒鞔鞯鞫鞣鞲鞴骱骰骷鹘骶骺骼髁髀髅髂髋髌髑魅魃魇魉魈魍魑飨餍餮饕饔髟髡髦髯髫髻髭髹鬈鬏鬓鬟鬣麽麾縻麂麇麈麋麒鏖麝麟黛黜黝黠黟黢黩黧黥黪黯鼢鼬鼯鼹鼷鼽鼾齄";
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                {
                    if (str[i].ToString().CompareTo("匝") >= 0)
                        return_py += "z";
                    else if (str[i].ToString().CompareTo("压") >= 0)
                        return_py += "y";
                    else if (str[i].ToString().CompareTo("昔") >= 0)
                        return_py += "x";
                    else if (str[i].ToString().CompareTo("挖") >= 0)
                        return_py += "w";
                    else if (str[i].ToString().CompareTo("塌") >= 0)
                        return_py += "t";
                    else if (str[i].ToString().CompareTo("撒") >= 0)
                        return_py += "s";
                    else if (str[i].ToString().CompareTo("然") >= 0)
                        return_py += "r";
                    else if (str[i].ToString().CompareTo("期") >= 0)
                        return_py += "q";
                    else if (str[i].ToString().CompareTo("啪") >= 0)
                        return_py += "p";
                    else if (str[i].ToString().CompareTo("哦") >= 0)
                        return_py += "o";
                    else if (str[i].ToString().CompareTo("拿") >= 0)
                        return_py += "n";
                    else if (str[i].ToString().CompareTo("妈") >= 0)
                        return_py += "m";
                    else if (str[i].ToString().CompareTo("垃") >= 0)
                        return_py += "l";
                    else if (str[i].ToString().CompareTo("喀") >= 0)
                        return_py += "k";
                    else if (str[i].ToString().CompareTo("击") >= 0)
                        return_py += "j";
                    else if (str[i].ToString().CompareTo("哈") >= 0)
                        return_py += "h";
                    else if (str[i].ToString().CompareTo("噶") >= 0)
                        return_py += "g";
                    else if (str[i].ToString().CompareTo("发") >= 0)
                        return_py += "f";
                    else if (str[i].ToString().CompareTo("蛾") >= 0)
                        return_py += "e";
                    else if (str[i].ToString().CompareTo("搭") >= 0)
                        return_py += "d";
                    else if (str[i].ToString().CompareTo("擦") >= 0)
                        return_py += "c";
                    else if (str[i].ToString().CompareTo("芭") >= 0)
                        return_py += "b";
                    else if (str[i].ToString().CompareTo("啊") >= 0)
                        return_py += "a";
                }
                else if (array[0] >= 215)    //二级汉字  
                {
                    return_py += ls_second_eng.Substring(ls_second_ch.IndexOf(str[i].ToString(), 0), 1);
                }
            }
                    ...
                    else
                    {
                        switch (asc)
                        {
                            ...
                            default:
                                ...
                                {
                                    if (_Value[i] <= asc) //判断汉字的拼音区编码是否在指定范围内
                                    {
                                        pystr += _Name[i];//如果不超出范围则获取对应的拼音
                                        break;
                                    }
                                }
                                break;
...
                }
        ...
        //定义拼音数组
        private static readonly string[] _Name = new string[]
             {
